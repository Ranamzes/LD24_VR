...
func _update_enable_press_to_continue():
	if is_inside_tree():
		...
		# Скрываем всю кнопку HoldButton, когда enable_press_to_continue включено
		var hold_button = $PressToContinue/HoldButton
		if hold_button:
			hold_button.visible = !enable_press_to_continue

		# Отключаем функциональность кнопки, даже если она видима
		if hold_button and hold_button.has_method("set_enabled"):
			hold_button.set_enabled(enable_press_to_continue)

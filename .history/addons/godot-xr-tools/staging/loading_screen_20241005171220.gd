...
func _update_enable_press_to_continue():
	# Assuming `press_to_continue_image` is the node for the image
	var press_to_continue_image = $Path/To/ImageNode
	if enable_press_to_continue:
		press_to_continue_image.hide()  # Hide the image when enabled
	else:
		press_to_continue_image.show()  # Show the image when not enabled

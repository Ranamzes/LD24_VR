...
func _update_enable_press_to_continue():
	if is_inside_tree():
		...
		# Скрываем изображение "Hold trigger to continue", когда enable_press_to_continue включено
		var hold_trigger_image = $PressToContinue/HoldButton/Viewport/HoldTrigger
		if hold_trigger_image:
			hold_trigger_image.visible = !enable_press_to_continue
